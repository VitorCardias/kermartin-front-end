...
const EditarCliente: React.FC<EditarClienteModalProps> = ({ isOpen, cliente, onClose, onEditar }) => {
  ...
  // Sincronizar formData quando o cliente muda e aplicar formatação
  useEffect(() => {
    const clienteFormatado = { ...cliente };
    if (tipoCliente === "PF" && "cpf" in clienteFormatado) {
      clienteFormatado.cpf = formatarCPFEnquantoDigita(clienteFormatado.cpf);
    } else if (tipoCliente === "PJ" && "cnpj" in clienteFormatado) {
      clienteFormatado.cnpj = formatarCNPJEnquantoDigita(clienteFormatado.cnpj);
    }
    clienteFormatado.numeroTelefoneContato = formatarTelefoneEnquantoDigita(clienteFormatado.numeroTelefoneContato);
    if (clienteFormatado.numeroWhatsAppContato) {
      clienteFormatado.numeroWhatsAppContato = formatarTelefoneEnquantoDigita(clienteFormatado.numeroWhatsAppContato);
    }
