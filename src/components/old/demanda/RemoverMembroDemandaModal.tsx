...
type AdicionarMembroModalProps = {
  membroEquipeDemanda: MembroEquipeDemanda | null;
  idDemanda: string;
  onClose: () => void;
  onRemovido: () => void;
};

const RemoverMembroDemandaModal: React.FC<AdicionarMembroModalProps> = ({ membroEquipeDemanda, onClose, onRemovido, idDemanda }) => {
  const { deletarMembroEquipe } = useEquipe(idDemanda);

  const removerMembro = async () => {
    if (membroEquipeDemanda) {
      try {
        await deletarMembroEquipe(membroEquipeDemanda.id);
    ...
    if (membroEquipeDemanda) { 
      return <>
        <div className="mt-3 mb-3">
          ...
          <p>Membro: {membroEquipeDemanda?.funcionarioDTO.nomeCompleto}</p>
        </div>
      ...
      return <>
        <div className="mt-3 mb-3">
          ...
          <p>Infelizmente, ao clicar no botão de remover, o sistema não conseguir reconhecer o membro que você deseja remover.</p>
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white p-6 rounded-md border border-gray-300 shadow-2xl max-h-screen overflow-y-auto">
        <h3 className="text-2xl font-semibold mb-3">Remover Membro da Equipe da Demanda</h3>
        {renderizarConteudo()}
      </div>
    </div>
  )
...
export default RemoverMembroDemandaModal;