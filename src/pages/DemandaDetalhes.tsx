...
const etapasMock = [
  { id: "1", titulo: "Coleta de Documentos", prazo: "22/02/2026", progresso: 33, ativa: true },
  { id: "2", titulo: "Análise da Documentação", prazo: "22/02/2026", progresso: 0, ativa: false },
  { id: "3", titulo: "Finalização", prazo: "22/02/2026", progresso: 0, ativa: false },
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[270px_minmax(0,1fr)_320px] gap-4">
        ...
        <aside className="bg-white border border-default rounded-xl p-4 sm:p-5 h-fit">
          <div className="flex flex-col sm:flex-row items-start justify-between gap-3 ">
            ...
            <button className="bg-primary text-white text-xs font-semibold px-3 py-2 rounded-lg cursor-pointer hover:brightness-110 transition">
              + Nova Etapa
            </button>
          </div>

          <div className="mt-5 space-y-3">
            {etapasMock.map((etapa, indice) => (
              <article
                key={etapa.id}
                className={`border rounded-xl p-4 ${etapa.ativa ? "border-blue" : "border-default"}`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="text-main font-bold">{indice + 1}. {etapa.titulo}</h3>
                    <p className="text-xs text-muted mt-1">Prazo: {etapa.prazo}</p>
                  </div>
                  <span className="text-xs text-muted font-semibold">{etapa.progresso}%</span>
                </div>
                <div className="mt-3 w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                  <div className="h-full bg-blue rounded-full" style={{ width: `${etapa.progresso}%` }} />
                </div>
              </article>
            ))}
          </div>
